...
class TwoStageTrAdaBoostR2:
    ...
    def fit(self, X_ori, y_ori, domain_col_ori, target_name, sample_weight=None):
        ...
        if sample_weight is None:
            # Initialize weights to 1 / n_samples
            sample_weight = np.empty(X_ori.shape[0], dtype=np.float64)
            sample_weight[:] = 1. / X_ori.shape[0]
        else:
            # Normalize existing weights
            sample_weight = sample_weight / sample_weight.sum(dtype=np.float64)
        ...
        source = concat_data[concat_data[:,-2]!=self.target_name]
        ...
        # get sample_size
        self.sample_size = np.array((len(source), len(target)))
        ...
        for istep in range(self.steps):
            ...
            for train, test in kf.split(X_target):
                ...
                cv_model = Stage2_TrAdaBoostR2(self.base_estimator,
...
if __name__ == "__main__":
    ...
    # normal twostage
    regr = TwoStageTrAdaBoostR2(random_state=np.random.RandomState(0), n_estimators=100, sample_size=sample_size)
    regr.fit(X, y)
